function [L, L1, L2] = Transfer_function_smallAR(mu,beta,inputs,kx,ky,sigma)
b = inputs.semichord;
x = inputs.x;
y = inputs.y;
z = inputs.z;
...
for k = 1:length(ky)
        for i = 1:length(kx)
          if  (ky(k)*b)^2 < (kx(i)*b*inputs.M/(beta))^2
                kapa = sqrt(mu(i)^2 - (ky(k)*b/beta)^2);
        ...
        %             1/sqrt(theta) .* exp(-1i*pi/4) .* erfz(2*1i*theta);
                 ...
                 L1(i,k) = (1/pi)*sqrt((2)/((kx(i)*b-1i*beta^2*kapa_p)*theta1_p))*...
                ...
                %L2 = -exp(1i*mu*(M-x/sigma)) ./ (pi*sqrt(2*pi*(kapa_p*beta^2+1i*k_x*b))) .* ...
                  ...
                  L2(i,k) = -exp(1i*mu(i)*(M-x/sigma)) ./ (pi*theta3_p.*sqrt(2*pi*(kapa_p*beta^2+1i*kx(i)*b))) .* ...
